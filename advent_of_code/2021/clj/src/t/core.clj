...
(defn -main
  ...
  (doseq [i (map inc (range 20))]
    (let [ns (symbol (str "t.day" i))
          ...
          input (parse data)]
      (print (format (str "Day %02d,") i))
      (flush)
      #_(print (format (str " part 1: %15d\n")
                     (part1 input)))
      (flush)
      #_(print (format (str "        part 2: %15d\n")
                     (part2 input)))
      (flush)
      (when true
        #_(print (format (str "        parse -> %14.3f\n")
                       (bench #(parse data))))
        (flush)
        #_(print (format (str "        part1 -> %14.3f\n")
                       (bench #(part1 input))))
        (flush)
        #_(print (format (str "        part2 -> %14.3f\n")
                       (bench #(part2 input))))
        (flush)
