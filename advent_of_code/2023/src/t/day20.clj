                ...
                (let [[level target origin] (peek todo)
                  ...
                  (case (:type m)
                    ...
                    :flip (case level
                            ...
                            :low (case (:state m)
                                   :on (recur (->> (:outputs m)
                                                   (map (fn [o] [:low o (:name m)]))
                                                   (reduce conj todo))
                                              (assoc-in state [(:name m) :state] :off)
                                              (update pulses :low + (count (:outputs m))))
                                   :off (recur (->> (:outputs m)
                                                    (map (fn [o] [:high o (:name m)]))
                                                    (reduce conj todo))
                                               (assoc-in state [(:name m) :state] :on)
