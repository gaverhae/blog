...
`jq` defines a small, specialized language for manipulating JSON values. It's
worth learning bits of that language, but you should not start your `jq`
journey by reading [the `jq` manual][manual]: that's more of a refrerence kind
of document.

There is an official [tutorial]; if you are convinced already, you can stop
reading this and go read that.
...
are used to working at the command-line. The semantics `|` is a bit like
chaining `flatMap` operations: conceptually, you first run all of the inputs
through the first filter, then collect all of the output (possibly more than
inputs), and run that through the second filter. (In practice, this is of
course done on the fly, not by collecting everything at each step.)
...
to `lein`, `aws` and `terraform`. Still, it was usful to be able to keep track
of state in a structured manner, and not having to come up with my own
Bash-friendly format was nice.
...
deployment). This JSON filter will take the existing content of the `deployed`
...
the result of parsin all the elements of the iniitial input as an array.

### Structured Bash state

Bash has arrays, but they're a bit clunky. Bash also does not have
dictionaries. But sometimes you do want a slightly structure value, while it's
still not quite worth moving up to a real language. `jq` can help here.
...
The `refresh_machines` filter is:
...
on just one line with no extra space, and the `-r` option prints JSON strings
without quotes.
...
It can take a bit of time to elarn, but it's tremendously useful and versatile.
