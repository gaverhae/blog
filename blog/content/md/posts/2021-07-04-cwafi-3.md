...
executed many times. Therefore, in this series I will assume that the cost of
parsing and optimizing are irrelevant because we can amortize them over many
evaluations.
...
it 30 times than 3, we have to invoke either garbage collection or memoization.
...
syntactic differences like the placement of parentheses), the benchmark result
is 2.7 <i>micro</i>seconds per run for the baseline and 5.34
<i>milli</i>seconds for the naive tree walking, or a factor of nearly 2000.
...
"functional" ones[^functional], we'll have first class functions, which means
it's easy to create new functions at runtime. And functions sound like exactly
what we need here. The main trick will be to try and make sure we compute as
much as we can before creating the function.
...
`rest` into functions, and then we can return anew function that calls those:
...
it's too lazy to take advantage of it here. I want to lean towards the former,
as adding "bangs" to the functions here doesn't help:
...
When a function is called, at runtime, there is a notion of what happens after
that function: it will return a value, and then something else will take that
value and use it. There is a style of programming in which this notion is
reified as something usually called a _continuation_. It's actually just a
normal function. Let's illustrate with an example.
...
This is not buying us much. However, if we wanted, we could know _give _add_
...
usual "direct" form: tree walking. You know, that thing we do all the time in
...
possibilities of errors. If we contrast with what we had before:
...
lazy: the `env0` on the right-hand side is indeed shadowing the `env0` passed
as an argument to the function definition in the `loop rest env0` expression,
...
If you haven't found it, here is a zoomed-in version:
...
While it's noce that getting back to a non-crashing version is that easy,
