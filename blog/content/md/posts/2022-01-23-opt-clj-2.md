...
Since the performance logs are collected through side-effects, you also need to
somehow _collect_ them. The simplest way of doing that is by using the
`profiled` macro instead of `profile`. The `profiled` macro will, in addition
to running the given expression with performance logging enabled, collect all
of the performance logs and return a vector of two elements, where the first
element is the return value of the given expression, and the second element is
the raw aggregated performance logs.

That's probably the best path if you're building a performance logging
framework into your application, but it's not the most convenient way to demo
in a blog post. Instead, here we're going to keep to the `profile` macro and
use the simple "print summary to stdout" adapter to collect the logs.
...
- The single slowest subfunction is `ends?`. It's a very short function to
  hopefully analyzing and improving it will be fairly straightforward.
...
running process to attach the profiler to. We cal just add a pause to our
`-main` function:
...
argument to the JAVM invocation.
...
So, yeah, I wasn't lying when I said profiling adds _some_ overhead.
...
generally hurts it a bit in circumstance where it does not allow us to
...
each function, the total time spent in it, and the lets you dig down into the
...
My tool of choice for Clojure benchmarking is [criterium]. At a first glance,
...
you're working with a different language I don't know why you've read so far,
...
instincts about performance, without having to compromise production code.
