...
code. This post is a summary of Clojure standard library from a performance
perspective.

### Big-O: what is it and when is it relevant?
...
- If your input is small, big-O analysis does not tell you anything. (What
  "small" means is, again, dependent on the algorithm.)
...
- Big-O sets "nest": if a function is big-O of \\(n\^2\\), it's also big-O of
  ...
  is sometimes called \\(\\Omega(f)\\). People often really mean \\(\\Omega\\)
  when they use \\(O\\).
...
[^transducer]: For the sake of simplicity, I am specifically ignoring the
...
function, which means that it is transforming the vector into a seq en then
